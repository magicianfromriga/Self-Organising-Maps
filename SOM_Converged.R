...
SOM <- function(x, input_grid) {
  ...
  for(ne in 1:n_epochs)
  {
    ...
    for (i in 1:n_iteration) # Looping through for training
    {
      ...
      if(isTRUE(all.equal(old_grid,input_grid)) | new_learning_rate<=0.01)
      {
        print(curr_i)
        print("Converged")
        breaker <- 1
...
t=1:length(l[[1]])
plot(t,l[[1]],xlab="Number of Epochs", ylab="Difference between Old and New Grid")
#rm(list=ls())
